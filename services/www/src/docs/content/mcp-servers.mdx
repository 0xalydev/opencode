...
| `auth_server_metadata_url` | Authorization-server metadata document used instead of protected-resource discovery. |

When the server publishes no usable authorization-server metadata, point `auth_server_metadata_url` at the metadata document directly:
...
The URL must use `https://` (`http://` is allowed only for loopback). Only use a metadata endpoint you trust.
